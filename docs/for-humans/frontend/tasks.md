...
- [x] 전적 목록 (순위 1위~8위, 덱 이름, 시간, 유닛+아이템 이미지) — LP 변동값 미제공 (Riot API 미지원)
- [x] 챔피언·아이템 이미지 호버 툴팁
- [x] 전적 행 클릭 → 8명 상세 패널 펼치기 (스테이지는 last_round 기반 Spring 변환값, 챔피언, 킬수, 잔여골드)
  - 순위 색상: 1위 금색, 2위 은색, 3위 동색
  - 시너지 아이콘: 소환사 카드 "많이 플레이한 시너지"와 동일한 CDN 이미지 소스 공유
  - 증강 비표시 (Riot API 미제공 확정)
- [x] 30개씩 더보기 버튼
- [x] 전적 업데이트 버튼 (UI)
- [ ] 전적 목록 게임 유형 배지 UI (랭크 / 일반)
- [ ] 전적 목록 게임 유형 필터 UI (전체 / 랭크 / 일반)
- [ ] 로딩 / 빈 상태 / 에러 상태 UI
...
- [ ] gameType 필드 API 연동 (Spring에서 queue_id 1100→랭크, 1090→일반 변환값)
...
- [ ] `api/guide.ts` 작성
  - [ ] 탭별 가이드 목록 조회 함수
  - [ ] 검색어, 탭, 페이지, 페이지 크기, 정렬, 코스트 필터 파라미터 전달
  - [ ] Community Dragon key 기반 이미지 URL 매핑
- [ ] `hooks/useGuide.ts` 작성
  - [ ] 활성 탭 상태 연결
  - [ ] 검색어 / 필터 / 정렬 / 페이지 상태 연결
  - [ ] React Query queryKey 분리
  - [ ] 검색어 또는 필터 변경 시 1페이지 초기화
  - [ ] 즐겨찾기 / 최근 확인한 가이드 상태 연결
...
- [ ] 접근성 점검
  ...
  - [ ] 챔피언 상세 모달 focus 처리
- [ ] QA 체크
  - [ ] 탭 전환 시 목록/검색/페이지 상태가 의도대로 유지 또는 초기화되는지 확인
  - [ ] 검색어 또는 필터 변경 시 1페이지로 초기화되는지 확인
  - [ ] 페이지당 표시 개수가 스펙과 일치하는지 확인
  - [ ] 이미지 로딩 실패 시 fallback이 표시되는지 확인
  - [ ] 즐겨찾기 해제 후 전체 목록으로 돌아갈 수 있는지 확인

> 작업 단위가 커질 경우 Guide는 UI 상태 보완, API/hook 분리, 실제 API 연결 PR로 나누어 올린다.
...
- [ ] `api/patchNotes.ts` 작성
  ...
  - [ ] 선택 패치 요약 조회 함수
  - [ ] 선택 패치 변경사항 목록 조회 함수
  - [ ] 검색어, 카테고리, 변경 타입, 영향도, 페이지, 페이지 크기 파라미터 전달
...
> 작업 단위가 커질 경우 PatchNotes는 API/hook 분리, 로딩/에러 상태 연결, 실제 API 연결 PR로 나누어 올린다.
