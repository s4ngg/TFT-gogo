...
- 전적 업데이트 버튼
  ...
  - 갱신 성공 시 쿨다운 동안 버튼 아래 "갱신됨" 빨간 텍스트 표시 (쿨다운 종료 시 자동 소멸)
  - 다른 소환사로 검색 이동 시에도 "갱신됨" 표시는 자동 리셋됨 (이전 소환사의 갱신 상태가 남지 않음)
  - 갱신 성공 후 60초 쿨다운: 버튼 비활성화 + 카운트다운 표시 (`N초 후 가능`). localStorage에 소환사별 마지막 갱신 시각 저장하여 페이지 재진입 시 잔여 쿨다운 복원
  - 쿨다운은 버튼 전용 — 429 rate limit과 별도 관리 (페이지를 RateLimitState로 가리지 않음)

---

## 많이 플레이한 시너지 / 챔피언

- 현재까지 불러온 전체 매치 기준 클라이언트 집계 (더 보기 클릭 시 재집계, 게임 수 제한 없음)
...
- 현재까지 불러온 전체 매치 기준으로 집계 (더 보기 클릭 시 실시간 재집계, 게임 수 제한 없음)
...
- 게임 날짜 및 경과 시간 표시 (예: `2026-06-10 · 3시간 전`). LP 변동값은 Riot API 미제공으로 표시하지 않음
