...
| **파라미터** | **타입** | **기본값** | **사용값** | **설명** |
| --- | --- | --- | --- | --- |
| start | int | 0 | 0 이상 | 시작 인덱스. 기본값 0. 0 미만 시 서비스 400 반환 |
| count | int | 20 | 1~20 | 반환할 매치 ID 수. 기본값 20. 1 미만 또는 20 초과 시 서비스 400 반환 |
| queue | int | - | 미사용 | 전적 검색은 queue 필터 파라미터 없이 1100·1090 두 번 조회 후 병합. 덱 집계 스케줄러는 별도 Key로 queue=1100 사용 |
...
| **HTTP 상태** | **의미** | **처리 방침** |
| --- | --- | --- |
| 400 | 잘못된 요청 (start < 0, count < 1, count > 20) | `ApiResponse.fail(message, 400)` 반환. Riot API 호출 없음 |
