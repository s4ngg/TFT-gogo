...
- JWT 설정은 `jwt.secret`과 `jwt.access-token-expiration-millis`를 사용한다. `jwt.secret`은 환경변수 `JWT_SECRET`으로 주입하며 32자 이상이어야 하고, 만료 시간은 `JWT_ACCESS_TOKEN_EXPIRATION_MILLIS`로 조정할 수 있다.
...
- 로그인 실패는 이메일 존재 여부가 노출되지 않도록 공통 인증 실패 응답으로 처리한다.
- 회원가입 중복 이메일과 사용자 조회의 존재하지 않는 사용자 오류는 필요한 범위에서 구분한다.
