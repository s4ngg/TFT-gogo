...
현재 `backend/build.gradle`에는 Flyway/Liquibase 의존성이 없다. `backend/spec-kit/constitution.md`도 migration 도구가 확정되기 전에는 테이블 변경 SQL, 적용 순서, 롤백 방법을 PR 또는 이슈에 명시하라고 정해두었다.
...
```text
backend/spec-kit/sql/guide-patchnote-local-schema.sql
backend/spec-kit/sql/guide-patchnote-local-seed.sql
backend/spec-kit/sql/guide-patchnote-local-reset.sql
```
...
아래 명령은 PowerShell 기준이다. `mysql < file.sql` 형태는 PowerShell에서 헷갈릴 수 있으므로 `source`를 사용한다.

```powershell
mysql --default-character-set=utf8mb4 -u tftuser -p tftgogo -e "source C:/Users/LHJ/Desktop/TFT-gogo/backend/spec-kit/sql/guide-patchnote-local-schema.sql"
mysql --default-character-set=utf8mb4 -u tftuser -p tftgogo -e "source C:/Users/LHJ/Desktop/TFT-gogo/backend/spec-kit/sql/guide-patchnote-local-seed.sql"
```
...
```powershell
cd C:/Users/LHJ/Desktop/TFT-gogo/frontend
npm run dev
```
...
```powershell
mysql --default-character-set=utf8mb4 -u tftuser -p tftgogo -e "source C:/Users/LHJ/Desktop/TFT-gogo/backend/spec-kit/sql/guide-patchnote-local-reset.sql"
```
...
#151 이후에는 SQL을 직접 넣는 방식에서 관리자 API 방식으로 넘어가는 것이 좋다.
