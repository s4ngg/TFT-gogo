...
- 공개 API는 프론트 `axiosInstance` 기준으로 `/api/guide`, `/api/guide/{tab}` 경로를 제공해야 한다.
...
- 공개 API는 프론트 `axiosInstance` 기준으로 `/api/patch-notes`, `/api/patch-notes/{version}/changes` 경로를 제공해야 한다.
