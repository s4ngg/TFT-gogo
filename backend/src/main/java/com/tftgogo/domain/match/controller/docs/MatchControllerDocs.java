...
public interface MatchControllerDocs {
    ...
    @Operation(
            summary = "매치 목록 조회",
            description = "PUUID 기준으로 start 오프셋부터 count 건의 매치를 조회합니다. Riot API 호출 간 200ms 쓰로틀이 적용됩니다. (queue_id 1090·1100만 포함)"
    )
    @ApiResponses({
    ...
    @Operation(
            summary = "매치 상세 조회",
            description = "matchId 기준으로 해당 게임에 참가한 8인 전체 데이터를 조회합니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공"),
